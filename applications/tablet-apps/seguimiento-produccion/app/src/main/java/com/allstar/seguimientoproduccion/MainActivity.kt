    ...
    fun cargarLotes(proyecto: ProyectoResponse, proceso: ProcesoResponse) {
        scope.launch {
            try {
                ...
                lotes = responseLotes.bodySeguro("GET /lotes")
                loteSeleccionado = lotes.firstOrNull()
                    ...
                    InstalacionMuebleCard(
                        ...
                        onReload = {
                            try {
                                ...
                                error = "Error al recargar mueble: ${e.message}"
                            }
                        },
                        onPiezaLocalUpdated = { piezaId, llegada ->
                            actualizarPiezaLocal(piezaId, llegada)
                        },
                        onMuebleLocalCompletado = { itemId ->
                            completarMuebleLocal(itemId)
                        }
                    )
                }
            }
