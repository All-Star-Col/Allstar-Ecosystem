...
_MANUAL_ENV = _read_env_file_manually(ENV_PATH)

for key, value in _MANUAL_ENV.items():
...
SCHEMA = os.getenv("DB_SCHEMA", "carpentry")
...
def get_responsables(
    ...
    if proceso_id is not None:
        return _all(
            ...
            WHERE activo = TRUE
              AND (proceso_id = :proceso_id OR proceso_id IS NULL)
            ORDER BY nombre, id
            """,
            {"proceso_id": proceso_id},
        )
...
def get_muebles_instalacion(
    ...
    for mueble in muebles:
        ...
        piezas_recibidas = sum(1 for p in piezas_item if _as_bool(p.get("llegada")))
        ...
        result.append(
            {
                ...
                "estado": mueble.get("estado"),
                "piezas": piezas_item,
                "herrajes": herrajes_item,
                "checklist_secciones": [
                    {"titulo": "Piezas", "tipo": "piezas", "items": piezas_item},
                    {"titulo": "Herrajes", "tipo": "herrajes", "items": herrajes_item},
                ],
                "piezas_total": piezas_total,
                "piezas_recibidas": piezas_recibidas,
                "herrajes_total": herrajes_total,
                "completo": piezas_total > 0 and piezas_total == piezas_recibidas,
            }
        )
