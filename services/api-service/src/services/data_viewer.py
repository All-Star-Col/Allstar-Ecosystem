...
PRODUCT_CATALOG_TABLES = {"base", "modelo", "referencia", "tela"}
...
class DataViewerService:
    ...
    async def patch_row(
        ...
        *,
        ...
        try:
            catalog_duplicate = await self._find_catalog_duplicate_for_update(
                table_config=table_config,
                normalized_pk=normalized_pk,
                normalized_changes=normalized_changes,
            )
            if catalog_duplicate:
                await self.db.execute(
                    text(
                        f"SET LOCAL statement_timeout = "
                        f"{int(DATA_VIEWER_MUTATION_TIMEOUT_MS)}"
                    )
                )
                merge_result = await self._merge_catalog_duplicate_in_current_transaction(
                    table_config=table_config,
                    source_catalog_id=normalized_pk.get("id"),
                    target_catalog_id=catalog_duplicate["id"],
                )
                await self.db.commit()
                logger.info(
                    (
                        "data_viewer.patch_row merged catalog duplicate "
                        "table_id=%s source_id=%s target_id=%s "
                        "updated_products=%s updated_orders=%s"
                    ),
                    request_data.table_id,
                    normalized_pk.get("id"),
                    catalog_duplicate["id"],
                    merge_result.get("updated_products", 0),
                    len(merge_result.get("updated_order_ids", [])),
                )
                return DataViewerRowUpdateResponse(
                    row=catalog_duplicate,
                    updated_columns=list(normalized_changes.keys()),
                )
            ...
            raise DBCommunicationError(
                f"Error merging catalog duplicate in DataViewer row: {error}"
            )
        ...
        try:
            ...
            synced_product_ids = await self._sync_product_names_for_catalog_change(
                table_config=table_config,
                normalized_pk=normalized_pk,
                normalized_changes=normalized_changes,
            )
            if synced_product_ids:
                logger.info(
                    "data_viewer.patch_row synced product names table_id=%s product_count=%s",
                    request_data.table_id,
                    len(synced_product_ids),
                )
    ...
    @staticmethod
    def _product_name_sql_expression(
        *,
        product_alias: str,
        product_columns_by_lower: dict[str, str],
    ) -> str:
        def catalog_label(
            *,
            table_name: str,
            ...
            resolved_product_column = product_columns_by_lower.get(product_column)
            if not resolved_product_column:
                return None

            ref_schema = _quote_identifier("data")
            ref_table = _quote_identifier(table_name)
            product_ref = f"{product_alias}.{_quote_identifier(resolved_product_column)}"
            if include_tela_reference:
                label_sql = (
                    "NULLIF(TRIM(CONCAT_WS(' ', "
                    "NULLIF(TRIM(COALESCE(ref.\"nombre\"::text, '')), ''), "
                    "NULLIF(TRIM(COALESCE(ref.\"referencia\"::text, '')), '')"
                    ")), '')"
                )
            else:
                label_sql = "NULLIF(TRIM(ref.\"nombre\"::text), '')"

            return (
                f"(SELECT {label_sql} "
                f"FROM {ref_schema}.{ref_table} ref "
                f"WHERE ref.\"id\" = {product_ref} "
                "LIMIT 1)"
            )

        referencia_1_column = (
            "id_referencia_1"
            if "id_referencia_1" in product_columns_by_lower
            else "id_referencia"
        )
        label_parts = [
            catalog_label(table_name="base", product_column="id_base"),
            catalog_label(table_name="modelo", product_column="id_modelo"),
            catalog_label(table_name="referencia", product_column=referencia_1_column),
            catalog_label(table_name="referencia", product_column="id_referencia_2"),
            catalog_label(table_name="referencia", product_column="id_referencia_3"),
            catalog_label(
                table_name="tela",
                product_column="id_tela",
                include_tela_reference=True,
            ),
        ]
        available_parts = [part for part in label_parts if part]
        if not available_parts:
            return "''"

        return f"TRIM(CONCAT_WS(' ', {', '.join(available_parts)}))"

    async def _sync_product_names_for_catalog_change(
        self,
        *,
        table_config: TableConfig,
        normalized_pk: dict[str, Any],
        ...
        if table_name not in PRODUCT_CATALOG_TABLES:
            return []

        changed_columns = {column.lower() for column in normalized_changes}
        if table_name == "tela":
            if changed_columns.isdisjoint({"nombre", "referencia"}):
                return []
        elif "nombre" not in changed_columns:
            return []

        catalog_id = normalized_pk.get("id")
        if catalog_id is None:
            return []

        product_table_name = await _find_existing_table(
            self.db,
            schema_name="data",
            table_candidates=PRODUCT_TABLE_NAME_CANDIDATES,
        )
        ...
        if (
            "id" not in product_columns_by_lower
            or "nombre" not in product_columns_by_lower
        ):
            return []

        affected_component_columns = [
            column_name
            for column_name, referenced_table in PRODUCT_COMPONENT_REFERENCE_TABLES.items()
            if referenced_table == table_name and column_name in product_columns_by_lower
        ]
        if not affected_component_columns:
            return []

        product_schema = _quote_identifier("data")
        product_table = _quote_identifier(product_table_name)
        product_id_column = _quote_identifier(product_columns_by_lower["id"])
        product_name_column = _quote_identifier(product_columns_by_lower["nombre"])
        product_alias = "p"
        name_expression = self._product_name_sql_expression(
            product_alias=product_alias,
            product_columns_by_lower=product_columns_by_lower,
        )
        where_sql = " OR ".join(
            (
                f"{product_alias}."
                f"{_quote_identifier(product_columns_by_lower[column_name])} = :catalog_id"
            )
            for column_name in affected_component_columns
        )
        set_clauses = [f"{product_name_column} = {name_expression}"]
        if "fecha_modificacion" in product_columns_by_lower:
            modification_column = _quote_identifier(
                product_columns_by_lower["fecha_modificacion"]
            )
            set_clauses.append(
                f"{modification_column} = CURRENT_TIMESTAMP"
            )

        result = await self.db.execute(
            text(
                f"""
                UPDATE {product_schema}.{product_table} AS {product_alias}
                SET {", ".join(set_clauses)}
                WHERE {where_sql}
                RETURNING {product_alias}.{product_id_column} AS id
                """
            ),
    ...
    async def _find_catalog_duplicate_for_update(
        self,
        *,
        table_config: TableConfig,
        normalized_pk: dict[str, Any],
        ...
        source_id = normalized_pk.get("id")
        ...
        if table_name == "tela":
            ...
            current_result = await self.db.execute(
                text(
                    f"""
                    SELECT *
                    FROM {_quote_identifier(table_config.schema_name)}.{_quote_identifier(table_config.table_name)}
                    WHERE "id" = :source_id
                    LIMIT 1
                    """
                ),
            ...
            current_row = dict(current_result.mappings().first() or {})
            if not current_row:
                return None

            next_name = str(
                normalized_changes.get("nombre", current_row.get("nombre") or "")
            ).strip()
            next_reference = str(
                normalized_changes.get(
                    "referencia",
                    current_row.get("referencia") or "",
                )
            ).strip()
            if not next_name and not next_reference:
                return None
            ...
            duplicate_result = await self.db.execute(
                text(
                    ...
                    FROM {_quote_identifier(table_config.schema_name)}.{_quote_identifier(table_config.table_name)}
                    WHERE "id" <> :source_id
                      AND LOWER(TRIM(COALESCE("nombre"::text, ''))) = LOWER(TRIM(:nombre))
                ...
                ),
                {
                    "source_id": source_id,
                    "nombre": next_name,
                    "referencia": next_reference,
                },
            )
        else:
            if "nombre" not in changed_columns:
                return None
            next_name = str(normalized_changes.get("nombre") or "").strip()
            if not next_name:
                return None
            ...
            duplicate_result = await self.db.execute(
                ...
                {"source_id": source_id, "nombre": next_name},
            )

        duplicate_row = duplicate_result.mappings().first()
        return dict(duplicate_row) if duplicate_row else None

    async def _merge_product_rows_in_current_transaction(
        self,
        *,
        ...
        updated_order_ids = [row["id"] for row in update_result.mappings().all()]
    ...
    async def _find_equivalent_product_for_component_values(
        self,
        *,
        ...
        match_columns = [
            column_name
            for column_name in PRODUCT_COMPONENT_COLUMNS
            if column_name in product_columns_by_lower
        ]
        if not match_columns:
            return None

        where_clauses = []
        params: dict[str, Any] = {"current_product_id": current_product_id}
        for index, column_name in enumerate(match_columns):
            resolved_column = product_columns_by_lower[column_name]
            key = f"component_value_{index}"
            where_clauses.append(
                f"{_quote_identifier(resolved_column)} IS NOT DISTINCT FROM :{key}"
            )
            params[key] = row_values.get(resolved_column)

        result = await self.db.execute(
            text(
                f"""
                SELECT "id", COALESCE("nombre"::text, '') AS nombre
                FROM {_quote_identifier("data")}.{_quote_identifier(product_table_name)}
                WHERE {" AND ".join(where_clauses)}
    ...
    async def _merge_catalog_duplicate_in_current_transaction(
        self,
        *,
        table_config: TableConfig,
        source_catalog_id: Any,
        target_catalog_id: Any,
    ) -> dict[str, Any]:
        table_name = table_config.table_name.lower()
        product_table_name = await _find_existing_table(
            self.db,
        ...
        if not product_table_name:
            return {"updated_products": 0, "updated_order_ids": []}
        ...
        if "id" not in product_columns_by_lower:
            return {"updated_products": 0, "updated_order_ids": []}

        affected_component_columns = [
            product_columns_by_lower[column_name]
            for column_name, referenced_table in PRODUCT_COMPONENT_REFERENCE_TABLES.items()
            if referenced_table == table_name and column_name in product_columns_by_lower
        ]
        if not affected_component_columns:
        ...
        product_select_columns = [
            product_columns_by_lower[column_name]
            for column_name in ("id", *PRODUCT_COMPONENT_COLUMNS, "nombre")
            if column_name in product_columns_by_lower
        ]
        where_sql = " OR ".join(
            f"{_quote_identifier(column_name)} = :source_catalog_id"
            for column_name in affected_component_columns
        )
        products_result = await self.db.execute(
            text(
                f"""
                SELECT {", ".join(_quote_identifier(column) for column in product_select_columns)}
                FROM {_quote_identifier("data")}.{_quote_identifier(product_table_name)}
                WHERE {where_sql}
                ORDER BY "id" ASC
                """
        ...
        affected_products = [dict(row) for row in products_result.mappings().all()]

        updated_products = 0
        updated_order_ids: list[Any] = []
        for product_row in affected_products:
            current_product_id = int(product_row[product_columns_by_lower["id"]])
            next_product_row = dict(product_row)
            changed_product_columns: dict[str, Any] = {}
            for component_column in affected_component_columns:
                if str(product_row.get(component_column)) == str(source_catalog_id):
                    next_product_row[component_column] = target_catalog_id
                    changed_product_columns[component_column] = target_catalog_id

            if not changed_product_columns:
                continue

            duplicate = await self._find_equivalent_product_for_component_values(
                product_table_name=product_table_name,
                product_columns_by_lower=product_columns_by_lower,
                row_values=next_product_row,
                current_product_id=current_product_id,
            )
            if duplicate:
                order_ids = await self._merge_product_rows_in_current_transaction(
                    source_product_id=current_product_id,
                    target_product_id=int(duplicate["id"]),
                )
                updated_order_ids.extend(order_ids)
                updated_products += 1
                continue

            recalculated_name = await self._build_product_name_from_components(
                next_product_row
            )
            if "nombre" in product_columns_by_lower:
                changed_product_columns[product_columns_by_lower["nombre"]] = (
                    recalculated_name
                )
            if "fecha_modificacion" in product_columns_by_lower:
                changed_product_columns[
                    product_columns_by_lower["fecha_modificacion"]
                ] = datetime.now(timezone.utc).replace(tzinfo=None)

            set_clauses: list[str] = []
            params = {"product_id": current_product_id}
            for index, (column_name, value) in enumerate(changed_product_columns.items()):
                key = f"product_set_{index}"
                set_clauses.append(f"{_quote_identifier(column_name)} = :{key}")
                params[key] = value

            await self.db.execute(
                text(
                    f"""
                    UPDATE {_quote_identifier("data")}.{_quote_identifier(product_table_name)}
                    SET {", ".join(set_clauses)}
                    WHERE "id" = :product_id
                    """
                ),
        ...
        await self.db.execute(
            text(
                f"""
                DELETE FROM {_quote_identifier(table_config.schema_name)}.{_quote_identifier(table_config.table_name)}
